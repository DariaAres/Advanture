...
namespace Rovie.Areas.Identity.Pages.Account.Manage
{
    public class EnableAuthenticatorModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!is2faTokenValid)
            {
                ModelState.AddModelError("Input.Code", "Код проверки недействителен.");
                await LoadSharedKeyAndQrCodeUriAsync(user);
                return Page();
            }
            ...
            _logger.LogInformation("Пользователь с ID '{UserId}' включил 2FA с помощью приложения аутентификатора.", userId);

            StatusMessage = "Ваше приложение аутентификатора было проверено.";
